 ...
 *      DateDimension 用户登录或者注册的时间-》解析为年月日天--》在日志中对应的时间为c_time
 *      PlatfromDimension 用户登录和注册的平台
 *      KpiDimension kpi 指标（new_user,browser_new_user,active_user）
 *
 *      TimeOutPutWriteable uid \ mid \sid
 */
public class NewUserMapper extends Mapper<LongWritable,Text,StatsUserDiemension,TimeOutPutWriteable> {
    public static final Logger logger=Logger.getLogger(NewUserMapper.class);
    ...
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        outPutWriteable.setId(split[6]);
        outPutWriteable.setTime(Long.valueOf(split[24]));

        //k->KpiDimension
        // kpi browser_new_user
        KpiDimension kpiDimension = new KpiDimension(kpiTypeEnum.BROWSER_NEW_USER.kpiName);
        ...
        //no kpi new_user
        KpiDimension kpinewUser = new KpiDimension(kpiTypeEnum.NEW_USER.kpiName);
