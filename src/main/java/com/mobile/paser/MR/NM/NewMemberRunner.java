...
public class NewMemberRunner implements Tool{
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setReducerClass(NewMemberReduce.class);
    ...
    private void computeTotalMember(Job job) {
        ...
        try {
            ...
            //查询上一天的新增总用户
            if(yesterdayId>0){
                String sql=job.getConfiguration().get(GlobalConstans.PREFIX_FIND_MEMBER+"DayId");
                ps=conn.prepareStatement(sql);
                ps.setInt(1, yesterdayId);
                rs= ps.executeQuery();
                while (rs.next()){
                    info.put(rs.getInt("platform_dimension_id")+"", rs.getInt("new_members"));
                }
            }
            if(nowDateId>0) {
                //查询今天的新增总用户
                String  nowsql=job.getConfiguration().get(GlobalConstans.PREFIX_FIND_MEMBER+"DayId");
                ...
                while (rs.next()){
                    ...
                    if(info.containsKey(platform_dimension_id+"")){
                        newmembers+=info.get(platform_dimension_id+"");
                    }
                    info.put(rs.getInt("platform_dimension_id")+"", rs.getInt(newmembers));
                }
            }
            for(Map.Entry<String,Integer> m:info.entrySet()){
                String total_sql=job.getConfiguration().get(GlobalConstans.PREFIX_INSERT+"total_members");
                ps.setInt(1, nowDateId);
