...
public class MyRecordWriter extends RecordWriter<StatsUserDiemension, ReduceOutPUtWritable> {
    ...
    private Map<kpiTypeEnum,PreparedStatement>  newMemberMap=new HashMap<>();
    ...
    public void write(StatsUserDiemension key, ReduceOutPUtWritable value) throws IOException, InterruptedException {
        ...
        int count=1;
        try {
            if(this.newMemberMap.containsKey(kpiName)){
                ps = this.newMemberMap.get(kpiName);
                ...
                //添加到缓存
                this.newMemberMap.put(kpiName, ps);
            }
            //将新的kpiname放入到缓存中
            ...
            if( this.kpiNumberMap.get(kpiName) % 50 ==0){
                this.newMemberMap.get(kpiName).executeBatch();
                this.conn.commit();
                this.newMemberMap.remove(kpiName);
            }
        } catch (Exception e) {
    ...
    public void close(TaskAttemptContext taskAttemptContext) throws IOException, InterruptedException {
        //关闭资源
        try {
            for(Map.Entry<kpiTypeEnum,PreparedStatement> en: this.newMemberMap.entrySet()){
